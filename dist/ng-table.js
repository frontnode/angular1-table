...
function($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn, ngTableEventsChannel) {
    ...
    function compileNoDataTemplate ($element) {
      var noDataTemplate = $element.find('[has-custom-nodata=true]');

      if (!noDataTemplate.length) {
        var noDataText = $attrs.noData;
        noDataTemplate = angular.element(document.createElement('div')).addClass('os-table-nodata').attr({
          'ng-show': '!$hasData',
          'ng-bind': '$noData'
        });
        $element.append(noDataTemplate);
      } else {
        noDataTemplate.attr({
          'ng-show': '!$hasData'
        });
      }
      $compile(noDataTemplate)($scope);
    }

    function compileOperationTemplate ($element) {
      var templateUrl = $attrs.tableOperationTemplate;
      var isOnlyBottom = !angular.isUndefined($attrs.isOnlyBottom);
      ...
      var operationWrapper = angular.element(document.createElement('div'))
        .addClass('os-table-operation-wrapper clearfix')
        .attr({
          'ng-show': 'params.total() > 0'
        });

      if (templateUrl) {
        var operationTemplate = angular.element(document.createElement('div')).attr({
          'ng-include': "'" + templateUrl + "'"
        });
        operationWrapper.append(operationTemplate);
      }
      operationWrapper.append(paginationTemplate);

      if (templateUrl && !isOnlyBottom) {
        var operationWrapperCopy = operationWrapper.clone(true);
        $element.prepend(operationWrapperCopy);
      }

      $element.append(operationWrapper);

      $compile(operationWrapper)($scope);

      if (!isOnlyBottom) {
        $compile(operationWrapperCopy)($scope);
      }
    }
    ...
    this.compileDirectiveTemplates = function () {
        ...
        if (!$element.hasClass('os-table')) {
            ...
            compileOperationTemplate($element);

            $scope.templates = {
                header: ($attrs.templateHeader ? $attrs.templateHeader : 'ng-table/header.html'),
                pagination: ($attrs.templatePagination ? $attrs.templatePagination : 'ng-table/pager.html')
            };
            $element.addClass('os-table');
            var headerTemplate = null;

            // $element.find('> thead').length === 0 doesn't work on jqlite
            var theadFound = false;
            angular.forEach(tableTemplate.children(), function(e) {
                if (e.tagName === 'THEAD') {
                    theadFound = true;
                }
            });
            if (!theadFound) {
                ...
                tableTemplate.prepend(headerTemplate);
            }
    ...
    function commonInit(){
        ...
        function bindDataToScope(params, newDatapage){
            if (params.settings().groupBy) {
                ...
                // Show no data
                if (!newDatapage || !newDatapage.length) {
                  $scope.$hasData = false;
...
app.directive('ngTablePaginationSelect', [
  function(){
    return {
      restrict: 'EA',
      replace: true,
      ...
      template: '<div class="os-select">' +
                    '<div class="select-btn gray3 clearfix" ng-click="showSelectList()">' +
                        '<div class="select-left">{{model}}</div>' +
                        '<div class="select-right">' +
                            '<span class="custom-caret" ng-class="{\'up\':isUpDropdown}"></span>' +
                        '</div>' +
                    '</div>' +
                    '<ul class="select-dropdown" ng-show="show" ng-class="{\'up\':isUpDropdown}">' +
                        '<li ng-class="{\'active-item\': $index == hoverIndex}" ng-repeat="value in items track by $index" ng-click="selectVal(value, $index)" ng-mouseover="hoverItem()">' +
                            '<div value="{{value}}">{{value}}</div>' +
                        '</li>' +
                    '</ul>' +
                    '<div class="modal-no-color" ng-show="show" ng-click="hideModal()"></div>' +
                '</div>',
      link: function (scope, elem, attrs) {
        var SELECT_ITEM_HEIGHT = 32,
            LEAST_EMPTY_HEIGHT = 310,
            SELECT_BOX_HEIGHT = 30;
        ...
        scope.selectVal = function (value, idx) {
          scope.model = value;
          if (scope.onChange()) {
            scope.onChange()(scope.model, idx)
          }
          scope.index = idx;
          scope.show = false;
        };
        ...
        scope.showSelectList = function () {
          if (!scope.direction) {
            var emptyHeight = $(window).height() + $(window).scrollTop() - elem.offset().top - SELECT_BOX_HEIGHT;
            if (emptyHeight >= (elem.find('.select-dropdown li').length * SELECT_ITEM_HEIGHT) || emptyHeight >= LEAST_EMPTY_HEIGHT) {
                scope.isUpDropdown = false;
            }
            else {
                scope.isUpDropdown = true;
            }
          } else {
            scope.isUpDropdown = scope.direction === 'up';
          }
          scope.show = !scope.show;
          scope.hoverIndex = scope.items.indexOf(scope.model);
        }
      }
    };
}]);
    ...
    function($compile, ngTableEventsChannel) {
        ...
        return {
            ...
            link: function(scope, element, attrs, ctrl) {
              var NUMBER_REGX = /^[1-9]\d*$/;
              var originalPage = 0;

              scope.page = {};

              // Focus out or enter will handle current page
              scope.handleCurrentPage = function(event) {
                var needHandlePage = (event && (event.keyCode === 13 || event.which === 13)) || !event
                if (needHandlePage) {
                  var currentPage = getValidCurrentPage(scope.page.currentPage);
              ...
              scope.changeSize = function(size, idx) {
                scope.params.count(size);
              }

              ngTableEventsChannel.onAfterReloadData(function(pubParams) {
                  scope.page = pubParams.generatePagesArray();
                  originalPage = scope.page.currentPage;
              ...
              function getValidCurrentPage(value) {
                if (value && NUMBER_REGX.test(value) && parseInt(value) <= scope.page.totalPages) {
                  return value;
                }
                return originalPage;
              }
...
angular.module('ngTable').run(['$templateCache', function ($templateCache) {
  ...
  $templateCache.put('ng-table/pager.html', '<div class="os-table-pagination" ng-show="params.settings().total > params.settings().counts[0]"><div class="pagesize"><span translate="display"></span><ng-table-pagination-select direction="params.settings().dropdownDirection" items="params.settings().counts" ng-model="page.pageSize" on-change="changeSize"></ng-table-pagination-select><span translate="per-page"></span></div><ul class="currentpage"><li class="page-btn" ng-class="{\'page-disabled\': !page.first.active}" ng-click="params.page(page.first.number)" translate="first_page"></li><li class="page-btn" ng-class="{\'page-disabled\': !page.prev.active}" ng-click="params.page(page.prev.number)"><span class="page-triangle prev"></span></li><li><input class="form-control" type="text" ng-model="page.currentPage" ng-blur="handleCurrentPage()" ng-keyup="handleCurrentPage($event)"/>/{{page.totalPages}}</li><li class="page-btn" ng-class="{\'page-disabled\': !page.next.active}" ng-click="params.page(page.next.number)"><span class="page-triangle next"></span></li><li class="page-btn" ng-class="{\'page-disabled\': !page.last.active}" ng-click="params.page(page.last.number)" translate="last_page"></li></ul></div>');
  $templateCache.put('ng-table/sorterRow.html', '<tr><th title="{{$column.headerTitle(this)}}" class="{{$column.class(this)}}" ng-repeat="$column in $columns" ng-click="sortBy($column, $event)" ng-if="$column.show(this)" ng-init="template=$column.headerTemplateURL(this)"><div ng-if="!template"><span translate="{{$column.title(this)}}"></span><span ng-class="{ \'sortable\': $column.sortable(this), \'asc\': params.sorting()[$column.sortable(this)]==\'asc\', \'desc\': params.sorting()[$column.sortable(this)]==\'desc\' }"></span></div> <div ng-if="template" ng-include="template"></div></th></tr>');
}]);
    return app;
}));
